...
//Based off of https://greentec.github.io/shadertoy-fire-shader-en/
...
vec2 turbulence(vec2 uv){
    ...
    //Creates a gradient to the flicker so the bottom doesn't move but the top of the flame does
...
void main()
{
    //candle flame
    if(flame == 0.0){
        //turbulece initially distorts the x value of the coordinate to give the fire a more realistic effect.
    ...
    if(flame == 1.0){
        ...
        float T3 = max(2.,1.25*strength)*time;
        q.x = mod(q.x,1.)-0.5;
        q.y += 0.65;
        float n = flameModifier(strength*q - vec2(0,T3));
        float c = 1.0 - 8. * pow( max( 0., length(q*vec2(1.8+q.y*1.5,.75) ) - n * max( 0.0, q.y+.75 ) ),1.8 );
        float c1 = n * c * (1.5-pow(1.20*uv.y,8.));
        c1=clamp(c1,0.,1.);
